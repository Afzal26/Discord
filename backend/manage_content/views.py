...
from backend.discort_bot.serializers import SoundClipSerializer, TagSerializer
from backend.discort_bot.models import SoundClip, Tag
from backend.manage_user.serializers import GuildSerializer, ProfileSerializer
from backend.manage_user.models import Guild, Profile
...
def fetch_data(request):
    ...
    collections = CollectionSerializer(profile.playlist, many=True).data
    sound_clips = SoundClipSerializer(SoundClip.object.all(), many=True).data
    tags = SoundClipSerializer(Tag.objects.all(), many=True).data

    data = {
        'roles': list(map(lambda x: x.name.lower(), list(get_user_roles(request.user)))),
...
def create_tag(request):
    ...
    Tag(title=name).save()
    return Response('Tag uploaded succesfully.', status=status.HTTP_200_OK)
...
def add_tag(request):
    ...
    if not Tag.objects.filter(title=tag_title).exists():
        return Response('Tag does not exists,', status=status.HTTP_404_NOT_FOUND)
    if not SoundClip.object.filter(name=sound_clip).exists():
    ...
    return Response("Tag '{tag}' succesfully added to '{clip}'.".format(clip=sound_clip, tag=tag_title),
...
def upload_sound_clip(request):
    ...
    with open(pathname, 'wb+') as temp_file:
        ...
        print(pathname)
        print(length)
        return Response("SoundClip '{}' uploaded succesfully".format(name))